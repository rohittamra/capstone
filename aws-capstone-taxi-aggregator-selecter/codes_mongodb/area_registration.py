...
@areanamespace.route('/getAreas/<area_id>', methods=['GET'])
...
@areanamespace.route('/addEditArea', methods=['POST'])
def addEdit_Area():
...
@areanamespace.route('/updateArea/<area_id>', methods=['PUT'])
def addEdit(area_id):
    if checkAreaIfExists(area_id,'active') == False:
        return json_response({"message": "Area Does Not Exists or it is InActive"})
...
def changeStatus(area_id):
    if checkAreaIfExists(area_id) == False:
        return json_response({"message": "Area Does Not Exists or it is InActive"})
    query = {"area_id": area_id}
    json_loaded = json.loads(request.data)
